...
const statusStyles: Record<string, { label: string; bg: string; text: string; border: string }> = {
  draft: { label: "Draft", bg: "bg-[#2A3A5C]", text: "text-[#D4A373]", border: "border-[#2A3A5C]" },
  ...
  return (
    <div className="card p-5">
      ...
      <div className="relative">
        ...
        <div className="absolute left-[5px] top-2 bottom-2 w-px bg-[#2A3A5C]" />
              ...
              <div key={rev.id} className="relative pl-6">
                ...
                >
                  <div className="flex items-center gap-2 mb-1">
                    <span className={`text-sm font-semibold ${isCurrent ? "text-brand-300" : "text-[#F4F1ED]"}`}>
                  ...
                  {rev.revision_notes && (
                    <p className="text-xs text-[#D4A373] mt-1 line-clamp-2">
                      {rev.revision_notes}
                    </p>
                  )}
                </Link>
