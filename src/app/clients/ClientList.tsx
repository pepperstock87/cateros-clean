              ...
              <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <Link
                    ...
                    className="inline-flex items-center gap-1.5 font-semibold text-[#F4F1ED] text-lg truncate hover:text-brand-300 transition-colors group/name"
                  >
                    {client.name}
                    <ChevronRight className="w-4 h-4 text-[#7A8BA8] group-hover/name:text-brand-300 transition-colors shrink-0" />
                  </Link>
                  <div className="flex flex-wrap gap-x-4 gap-y-1 mt-2 text-sm text-[#D4A373]">
                    {client.email && (
                      <a
                        href={`mailto:${client.email}`}
                        className="inline-flex items-center gap-1.5 hover:text-[#F4F1ED] transition-colors"
                      >
                  ...
                  <div className="flex flex-wrap gap-1.5 mt-3">
                    {Object.entries(client.statusDistribution).map(([status, count]) => (
                      <span
                        key={status}
                        className={`inline-flex items-center px-2 py-0.5 rounded text-xs font-medium ${statusColors[status] ?? "bg-[#2A3A5C] text-[#D4A373]"}`}
                      >
                        {count} {status}
                      </span>
                    ))}
                ...
                <div className="flex flex-row sm:flex-col items-end gap-3 sm:gap-2 text-right shrink-0">
                  <div className="flex items-center gap-1.5 text-sm text-[#F4F1ED]">
                    <DollarSign className="w-3.5 h-3.5 text-[#D4A373]" />
                    <span className="font-medium">{formatCurrency(client.totalRevenue)}</span>
                  </div>
                  <div className="flex items-center gap-1.5 text-sm text-[#D4A373]">
