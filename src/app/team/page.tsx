    ...
    <div className="p-4 md:p-8 max-w-4xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6 md:mb-8">
        <div>
          ...
          {org && (
            <p className="text-sm text-[#D4A373] mt-1">{org.name}</p>
          )}
        </div>
        ...
        <div className="rounded-xl border border-[#2A3A5C] bg-[#182030] p-5 mb-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            {org.primary_contact_email && (
              <div>
                <span className="text-[11px] uppercase tracking-wider text-[#7A8BA8] block mb-1">Primary Contact</span>
                <span className="text-sm text-[#F4F1ED]">{org.primary_contact_email}</span>
              </div>
            )}
          </div>
          <div className="mt-3 pt-3 border-t border-[#2A3A5C]">
            <span className="text-xs text-[#7A8BA8]">{members.length} member{members.length !== 1 ? "s" : ""}</span>
          </div>
        </div>
      )}
