...
export default function NewRecipePage() {
  ...
  return (
    <div className="p-4 md:p-8 max-w-3xl mx-auto">
      <Link href="/recipes" className="inline-flex items-center gap-1.5 text-sm text-[#D4A373] hover:text-[#F4F1ED] mb-3 transition-colors">
        <ArrowLeft className="w-4 h-4" /> All recipes
      </Link>
      <h1 className="font-display text-xl md:text-2xl font-semibold">New Recipe</h1>
      <p className="text-sm text-[#D4A373] mt-1 mb-6">Add a recipe with ingredients and costing.</p>
      ...
      <form action={action} className="space-y-6">
        ...
        <div className="card p-6">
          ...
          {ingredients.length === 0 ? (
            <p className="text-sm text-[#7A8BA8] text-center py-6">No ingredients yet</p>
          ) : (
              ...
              {ingredients.map((ing, idx) => (
                <div key={ing.id} className="flex items-start gap-2">
                  ...
                  <div className="relative">
                    <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[#7A8BA8]">$</span>
                    <input
                      type="number"
        ...
        <div className="card p-6">
          ...
          {casePrice && unitsPerCase && (
            <div className="mt-4 pt-4 border-t border-[#2A3A5C] grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
          ...
          <div className="card p-5">
            <div className="grid grid-cols-3 gap-4 text-center">
              <div>
                ...
                <div className="text-xs text-[#D4A373] mt-0.5">cost per serving</div>
              </div>
              <div>
